...
public class DataStream<T> {
	...
	public <R> SingleOutputStreamOperator<R> flatMap(FlatMapFunction<T, R> flatMapper) {
		...
		// 事实上，每一个 Transformation 都对应了一个 StreamOperator <重要> <封装用户代码的，open、close等方法就定义在这里>
	...
	public <R> SingleOutputStreamOperator<R> transform(String operatorName, TypeInformation<R> outTypeInfo, OneInputStreamOperator<T, R> operator) {
		...
		//第一步在617 行
		//第二步：由于flatMap这个操作只接受一个输入，所以再被进一步包装为 OneInputTransformation。
		OneInputTransformation<T, R> resultTransform = new OneInputTransformation<>(
				this.transformation, //input transFormation，也就是上一个 env.socketTextStream 创建的transformation 被当做新创建的 transFormation的input，最终把全部的 transFormation 串起来；
				operatorName,
				operator,  //这个Operator指的是 StreamOperator，所以这里有个三级依赖关系，DataStream -> transformation -> StreamOperator
						  //这里有个注意点，SingleOutputStreamOperator这个是DataStream的子类，代表一个流(这个起名太扯了)，
						// 而StreamOperator是封装用户代码的(open,close,snapshot等方法)基本每个算子都对应一个StreamOperator，flatMap->StreamFlatMap, map->StreamMap,
						//StreamFlatMap,StreamMap 这些StreamOperator大部分都是基于 AbstractUdfStreamOperator 来实现的；
				outTypeInfo,
				environment.getParallelism());

		//这里又返回一个新的 DataStream，environment 和 transFormation 也设置好了，后续就是一直这个逻辑呗。。。
