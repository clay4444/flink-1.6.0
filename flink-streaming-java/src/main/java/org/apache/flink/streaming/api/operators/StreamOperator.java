...
/**
 * 封装 用户写的自定义的处理函数，map，filter 等的类，
 * 两个子接口 主要提供了操作数据流中具体元素的方法，processElement、processWatermark、processLatencyMarker 等；
 * 	1.TwoInputStreamOperator
 * 	2.OneInputStreamOperator 更常用；
 *
 * 而 AbstractUdfStreamOperator 这个抽象子类则提供了自定义处理函数对应的算子的基本实现:
 *  基本实现了open close snapshot initState 等这些功能，
 */
@PublicEvolving
