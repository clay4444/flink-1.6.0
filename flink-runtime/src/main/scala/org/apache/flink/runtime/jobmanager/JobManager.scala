...
class JobManager(
  ...
  override def handleMessage: Receive = {
    ...
    //获得leader授权，将自身被分发到的 session id 写到 zookeeper，并 恢复所有的 jobs
    case GrantLeadership(newLeaderSessionID) =>
      ...
      // 剥夺leader授权，打断并清空所有的 job 信息，但是保留作业缓存，注销所有的 TaskManagers
    ...
    case msg: ReconnectResourceManager =>
      ...
      // 注册 TaskManager，如果之前已经注册过，则只给对应的 Instance 发送消息，否则启动注册逻辑：
    // 在 InstanceManager 中注册该 Instance 的信息，并停止 Instance BlobLibraryCacheManager 的端口【供下载 lib 包用】，
   ...
   */
  /**
    * 最后一项SubmintJob就是我们要关注的，从客户端收到JobGraph，转换为 ExecutionGraph并执行的过程。
    * @param jobGraph
    * @param jobInfo
    * @param isRecovery
    */
